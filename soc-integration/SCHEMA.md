# Sentrix SOC alert schema, version 1.0

This document is the single source of truth for Sentrix SOC alerts. Producers
...
| `source` | string | yes | Routing discriminator; exactly `sentrix`. |
...
correlation engines, or other methodologies. Sentrix is an ML/physical-sensing
detection engine. `Incident Finding` is not used because a raw Sentrix alert has
not yet entered an incident-management workflow. There is no core OCSF physical
intrusion event class, so Sentrix-specific physical context is retained under
the standard `unmapped.sentrix` object rather than invented as top-level OCSF
attributes.
...
canonical binary64 remains available under `unmapped.sentrix.confidence`.
